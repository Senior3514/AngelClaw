# ANGELGRID Threat Model – AI Agent Threats

## Threat Categories

### 1. Prompt Injection
- **Description**: Malicious instructions injected into AI agent context via user input, retrieved documents, or tool outputs.
- **ANGELGRID Mitigation**: AI Shield adapter inspects tool-call arguments for known injection patterns. Policy rules can block tool calls with suspicious payloads.
...
- **ANGELGRID Mitigation**: Network and file-access events are evaluated against policy rules. Outbound POST to untrusted destinations triggers alerts/blocks.
...
- **ANGELGRID Mitigation**: The `accesses_secrets` heuristic in the AI Shield flags tool calls that reference credential-related terms. Policy rules block these by default.
- **OWASP AI Reference**: LLM06 – Sensitive Information Disclosure
...
- **ANGELGRID Mitigation**: Shell and system events with elevated privilege indicators are evaluated. High-risk tool names (bash, sudo, ssh) get elevated severity.
...
- **ANGELGRID Mitigation**: Future rate-limiting sensors. Current: alert rules for rapid event generation from a single agent.
...
- **ANGELGRID Mitigation**: Policy rules can allowlist specific tool names and block unknown tools. Future: tool signature verification.
