...
def mindlink_download_intel(
    listing_id: str, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    from cloud.services.intel_marketplace import intelMarketplaceService_service
...
def mindlink_generate_executive(
    tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), time_range_hours: int = 24
):
...
def mindlink_generate_technical(
    incident_id: str, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    from cloud.services.report_generator import reportGeneratorService_service
...
def mindlink_generate_compliance(
    tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), framework: str = "soc2"
):
    from cloud.services.report_generator import reportGeneratorService_service
...
@router.get("/status")
def mindlink_status_2(tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
    from cloud.services.report_generator import reportGeneratorService_service
