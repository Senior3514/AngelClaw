...
def create_segment(
    req: SegmentCreateRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
...
def evaluate_segment(
    ...
    return microsegmentation_engine.evaluate_access(
        tenant_id,
        {"zone": source} if source else {},
        {"zone": target} if target else {},
        protocol,
    )
...
def create_identity_policy(
    req: IdentityPolicyRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    ...
    return identity_policy_service.create_policy(
        ...
        req.name,
        req.identity_type,
        req.identity_pattern,
        req.resource_pattern,
        req.action,
        req.conditions,
        req.priority,
    )
...
def assess_device(
    req: DeviceAssessRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
...
def assess_session(
    req: SessionAssessRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
