...
def apex_start_pentest(tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), target: str, scope: str = 'internal'):
...
def apex_get_findings(tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), run_id: str):
    from cloud.services.pentest_auto import pentestAutoService_service
    return pentestAutoService_service.get_findings(tenant_id, run_id)

@router.post("/verify-remediation")
def apex_verify_remediation(tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), finding_id: str):
    from cloud.services.pentest_auto import pentestAutoService_service
...
def apex_create_campaign(tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), req: dict = {}):
...
def apex_execute_phase(tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), campaign_id: str, phase: int = 0):
    from cloud.services.red_team import redTeamService_service
    return redTeamService_service.execute_phase(tenant_id, campaign_id, phase)

@router.get("/get-gaps")
def apex_get_gaps(tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), campaign_id: str):
    from cloud.services.red_team import redTeamService_service
