...
def agi_analyze(req: ThreatAnalysisRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
...
def response_trigger(req: ResponseTriggerRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
    ...
    return autonomous_response_service.trigger_response(
        tenant_id=tenant_id, incident_id=req.incident_id, response_type=req.response_type,
    )
...
def response_override(req: ResponseOverrideRequest):
    ...
    return autonomous_response_service.override_response(
        response_id=req.response_id, operator=req.operator, reason=req.reason,
    )
...
def federation_join(req: FederationJoinRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
    from cloud.services.threat_federation import threat_federation_service
    return threat_federation_service.join_federation(
        tenant_id=tenant_id, org_name=req.org_name, trust_level=req.trust_level,
    )


@router.post("/federation/share")
def federation_share(req: FederationShareRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
    from cloud.services.threat_federation import threat_federation_service
...
def federation_consume(min_trust: float = 0.5, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
    from cloud.services.threat_federation import threat_federation_service
...
def soc_triage(req: TriageRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
    ...
    return soc_autopilot_service.triage_alert(
        tenant_id=tenant_id, alert_id=req.alert_id, alert_data=req.alert_data,
    )


@router.post("/soc/investigate/{investigation_id}")
def soc_investigate(investigation_id: str, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
    from cloud.services.soc_autopilot import soc_autopilot_service
    return soc_autopilot_service.investigate(tenant_id, investigation_id)


@router.post("/soc/assign")
def soc_assign(req: AssignAnalystRequest, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")):
    from cloud.services.soc_autopilot import soc_autopilot_service
    return soc_autopilot_service.assign_analyst(
        tenant_id=tenant_id, alert_id=req.alert_id, analyst_id=req.analyst_id,
    )
