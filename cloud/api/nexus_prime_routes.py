...
def nexus_scan_crypto(
    targets: list[str] | None = None, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    if targets is None:
        targets = []
...
def nexus_create_migration(
    plan_data: dict | None = None, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
...
def nexus_generate_keypair(
    algorithm: str = "kyber-1024", tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    from cloud.services.quantum_crypto import quantum_crypto_service
...
def nexus_surface_changes(
    since_hours: int = 24, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    from cloud.services.attack_surface import attack_surface_service
...
def nexus_analyze_request(
    request_data: dict | None = None, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
...
def nexus_add_rasp_rule(
    rule: dict | None = None, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    if rule is None:
        rule = {}
    from cloud.services.runtime_protection import runtime_protection_service
