...
def sentinel_eye_detect_anomalies(
    tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), time_window_minutes: int = 60
):
    from cloud.services.log_analytics import logAnalyticsService_service
...
def sentinel_eye_search_logs(
    query: str, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID"), limit: int = 50
):
    from cloud.services.log_analytics import logAnalyticsService_service
...
def sentinel_eye_create_span(
    ...
    return distributedTracingService_service.create_span(
        tenant_id, trace_id, service, operation, parent_span_id
    )


@router.get("/get-trace")
def sentinel_eye_get_trace(
    trace_id: str, tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    from cloud.services.distributed_tracing import distributedTracingService_service
...
def sentinel_eye_correlate_events(
    event_ids: list[str], tenant_id: str = Header("dev-tenant", alias="X-TENANT-ID")
):
    from cloud.services.distributed_tracing import distributedTracingService_service
