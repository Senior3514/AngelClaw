...
def predict_threat_vectors(
    ...
    if ai_count > 0 and len(secret_events) > 0:
        ...
        predictions.append(ThreatPrediction(
            ...
            confidence=round(confidence, 2),
            rationale=(
                f"Detected {ai_count} AI tool calls with {len(secret_events)} secret-access attempts — "
                "pattern suggests lateral movement via AI agent credential harvesting."
            ),
    ...
    if file_count > 0 and shell_count > 0:
        ...
        predictions.append(ThreatPrediction(
            ...
            rationale=(
                ...
                "file modifications combined with shell activity may indicate persistence mechanisms."
            ),
