...
async def run_guardian_scan(
    ...
    stale_agents = [
        ...
        if a.status == "active"
        and a.last_seen_at
        and a.last_seen_at.replace(tzinfo=None) < stale_cutoff.replace(tzinfo=None)
    ]
    if stale_agents:
        risks.append(
            ScanRisk(
    ...
    secret_access_events = [
        e for e in all_recent if (e.details or {}).get("accesses_secrets") is True
    ]
    ...
    total_checks += 1
    from cloud.auth.config import ADMIN_PASSWORD, AUTH_ENABLED
    ...
    if not AUTH_ENABLED:
        risks.append(
            ScanRisk(
                ...
                description=(
                    "The Cloud API has no authentication. Anyone with"
                    " network access can read and modify data."
                ),
                severity="critical",
    ...
    if not webhook_url:
        risks.append(
            ScanRisk(
                ...
                description=(
                    "Critical alerts are only visible in the dashboard."
                    " Configure a webhook for external alerting."
                ),
                severity="low",
            ...
            if integrity.get("drifted", 0) > 0:
                risks.append(
                    ScanRisk(
                        ...
                        description=(
                            "AngelClaw core modules have been modified."
    ...
    memory_risk_events = [
        ...
        or "context" in str(e.details or {}).lower()
        and "overflow" in str(e.details or {}).lower()
    ]
