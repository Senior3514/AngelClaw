...
class TestThreatHuntingService:
    def test_execute_query(self, db):
        ...
        result = threat_hunting_service.execute_query(
            db, "dev-tenant", {"filters": {"category": "shell"}, "time_range_hours": 24}
        )
    ...
    def test_execute_with_severity_filter(self, db):
        ...
        result = threat_hunting_service.execute_query(
            db, "dev-tenant", {"filters": {"severity": "high"}, "time_range_hours": 24}
        )
        assert result["total_matches"] >= 0
    ...
    def test_execute_with_group_by(self, db):
        _seed_events(db, 5)
        result = threat_hunting_service.execute_query(
            db, "dev-tenant", {"filters": {}, "time_range_hours": 24, "group_by": "category"}
        )
    ...
    def test_execute_with_limit(self, db):
        _seed_events(db, 20)
        result = threat_hunting_service.execute_query(
            db, "dev-tenant", {"filters": {}, "time_range_hours": 24, "limit": 5}
        )
    ...
    def test_run_saved_query(self, db):
        ...
        result = threat_hunting_service.run_saved_query(db, "dev-tenant", saved["id"])
        assert "total_matches" in result

    def test_run_nonexistent_query(self, db):
        result = threat_hunting_service.run_saved_query(db, "dev-tenant", "fake-id")
        assert "error" in result

    def test_execute_empty_results(self, db):
        result = threat_hunting_service.execute_query(
            db, "dev-tenant", {"filters": {"category": "nonexistent"}, "time_range_hours": 1}
        )
        assert result["total_matches"] == 0
